package com.redevance.resources.conf;
...
public class Init {
    ...
    private Init(){
        try {
            File f=new File(Paths.get(System.getenv("ProgramData"),"redevance","conf","init.properties").toString());
            if(!f.exists()){
                new File(Paths.get(System.getenv("ProgramData"),"redevance","conf").toString()).mkdirs();
                f.createNewFile();
            }
            file= Files.newInputStream(Paths.get(System.getenv("ProgramData"),"redevance","conf","init.properties"));
            ini.load(file);
